...
def processFiles(pos_dir, neg_dir, recurse=False, output_file=False,
    ...
    @param recurse (bool): Traverse directories recursively (else, top-level only). 是否递归处理子目录的文件
    @param output_file (bool): Save processed samples to file. 是否将处理结果保存到文件
    @param output_filename (str): Output file filename.
    @param color_space (str): Color space conversion. 描述图片的颜色空间，默认为bgr，还可以选gray&hsv(色相，饱和度，色调)
    @param channels (list): Image channel indices to use. 颜色通道数
    ...
    # hog_features 是否提取hog特征，通过计算和统计图像局部区域的梯度方向直方图来构成特征
    # hist_features 是否提取颜色直方图特征，通过对图像的颜色分布进行统计来描述图像的特征
    # spatial_features 空间特征通常是指图像的像素值在空间上的分布
    ...
    bar = tqdm.tqdm(total=len(pos_files) + len(neg_files), desc="Convert process") # 进度条
    for i, filepath in enumerate(pos_files + neg_files):
        image = cv2.imread(filepath)
        if cv_color_const > -1: # 转换颜色空间
            image = cv2.cvtColor(image, cv_color_const)

        if len(image.shape) > 2:
            image = image[:, :, channels]

        feature_vector = descriptor.getFeatureVector(image) # 提取图像特征向量
...
def trainSVM(filepath=None, feature_data=None, C=1,
    ...
    # classifier = svm.LinearSVC(**best_params)
