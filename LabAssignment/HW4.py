...
class Conv2d(Layer):
    ...
    def backward(self, da):
        ...
        db = da.sum((0, 2, 3)) / batch_size  # bias 梯度，每个输出通道的平均梯度，用于更新bias
        ...
        for i in range(h_out):
            ...
            for j in range(w_out):
                ...
                dw += np.sum(x_slice[:, np.newaxis, :, :, :] * da_curr, axis=0)
                # dw += np.sum(
                ...
                #         out[k, :, h_start:h_end, w_start:w_end] += self.w[c, :, :, :] * da[k, c, i, j]
                out[:, :, h_start:h_end, w_start:w_end] += np.sum(self.w * da_curr, axis=1)
                # out[:, :, h_start:h_end, w_start:w_end] += np.sum(self.w * da[:, :, i:i + 1, j:j + 1], axis=1)
...
def train(model: SequentialModel, epochs: int,
    ...
    for epoch in range(epochs):
        for idx in range(train_loader.length):
            ...
            model.zero_grad()
            assert len(x_batch.shape) == 4, "Input to model should be 4D array"
            assert x_batch.shape[1] == 1, "Channel dimension mismatch for first Conv2d layer"
...
def main():
    ...
    X_train = np.empty((num_train, h, w), dtype=float)  # (400, 28, 28)
    y_train = np.empty(num_train, dtype=int)  # (400)
    X_test = np.empty((num_test, h, w), dtype=float)  # (100, 28, 28)
    y_test = np.empty(num_test, dtype=int)  # (100)
    for b in range(num_train):
        img = [input().split() for _ in range(h)]  # (28, 28)
        X_train[b] = np.array(img, dtype=int) / 255  # (400, 28, 28)
        y_train[b] = int(input())
    ...
    X_train = X_train[:, np.newaxis, :, :]  # (400, 1, 28, 28) num_test * 1 * h * w 通道数为1，是灰度图像
    X_test = X_test[:, np.newaxis, :, :]  # (100, 1, 28, 28)

    # one-hot encoding for training.
    one_hot_train = np.zeros((len(y_train), num_classes), dtype=float)  # (400, 10) 全0的二维数据 num_train * num_classes
    one_hot_train[np.arange(len(y_train)), y_train] = 1  # 生成一个从 0 到 len(y_train) - 1 的数组，表示 y_train 中每个样本的索引
    ...
    model = SequentialModel([
        ...
        FullyConnected(588, num_classes),
        # complete your code here
        Softmax()
    ])
